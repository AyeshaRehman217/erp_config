...
public class UpdateStateHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (value.containsKey("stateCode")) {
                        if (value.getFirst("stateCode").isEmpty()) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                            ...
                            if (!value.getFirst("stateCode").matches("^[\\sa-zA-Z0-9*.,!@#$&()_-]*$")) {
                                messages.add(
                                        new AppResponseMessage(
                                                AppResponse.Response.ERROR,
                                                "Invalid State Code"
                    ...
                    if (value.containsKey("type")) {
                        if (!value.getFirst("type").matches("^[\\sa-zA-Z0-9*.,!@#$&()_-]*$")) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Invalid Type"
